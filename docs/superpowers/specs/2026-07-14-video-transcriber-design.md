...
2. **Every use:** drag one or more video files onto the **Transcribe** app icon.
   - A small popup asks the language: type `hr` (Croatian) or `sl` (Slovenian).
     Defaults to `hr`.
   - Each video is transcribed in turn.
   - A macOS notification reports progress and completion.
...
- `on open theFiles`: catch dropped files.
- Show the language popup (`display dialog … default answer "hr"`), validate the
  answer is `hr` or `sl`.
- Show a "working…" notification, invoke `bin/transcribe` with the language and
  file paths, show a "done" (or "failed") notification.
